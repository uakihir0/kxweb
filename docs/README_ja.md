...
- **認証**: Cookie (`auth_token`/`ct0`)、OAuth1、ゲストトークンの各方式に加え、エンドポイント単位でレート制限を追跡するマルチセッションプール
- **タイムライン操作**: ホームタイムライン、ユーザーのツイート、リストタイムライン、いいね、ブックマーク
- **ツイート操作**: 単一ツイート・会話スレッド・返信の取得、ツイートの作成と削除
- **エンゲージメント**: いいね／いいね解除、リツイート／リツイート解除
- **検索**: ツイート検索およびユーザー検索 (`SearchTimeline`)
- **ユーザー操作**: プロフィール、フォロー／フォロワー、アカウント情報
- **トレンド**: 地域別のトレンド (`getTrendLocations`/`getTrends`)
- **記事**: ツイートに埋め込まれた X の記事 (Article) の読み取り
- **Explore・メディア**: ニュースタイムライン、チャンク方式のメディアアップロード

詳細な API 使用例は `core/src/jvmTest` のテストコードを参照してください。

#### 認証について

読み取り専用のアクセスには X アカウントは必須ではありません。`XWebFactory.instanceGuest()`
はゲストトークンを自動取得し、公開プロフィール・単一ツイート・トレンドを読み取れます。
検索と会話スレッドはログイン済みセッションが必要で、ブラウザの Cookie を使う
`XWebFactory.instance(authToken, csrfToken)` または `XWebFactory.instanceOAuth(...)` を利用してください。
